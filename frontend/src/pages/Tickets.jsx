...
export default function Tickets() {
  ...
  const getStatusClass = (status) => {
    switch (status) {
      ...
      case "Waiting For Sources": return "bg-purple-100 text-purple-700";
      case "Waiting For Approval": return "bg-orange-100 text-orange-700";
  ...
  const stats = {
    ...
    pendingApproval: tickets.filter((t) => t.status === "Pending Approval" || t.status === "Waiting For Approval").length,
  ...
  return (
    <MainLayout>
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-8">
        ...
        <button onClick={() => navigate("/create-ticket")} className="bg-black text-white px-5 py-3 rounded-xl w-full lg:w-auto hover:bg-gray-800">
          + Create Ticket
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-5 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <select value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value)} className="border p-3 rounded-xl">
            <option>All</option>
          ...
          <select value={divisionFilter} onChange={(e) => setDivisionFilter(e.target.value)} className="border p-3 rounded-xl">
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr className="text-left">
              <th className="p-5 cursor-pointer" onClick={() => handleSort("title")}>Title</th>
              <th className="p-5 cursor-pointer" onClick={() => handleSort("division")}>Division</th>
              <th className="p-5 cursor-pointer" onClick={() => handleSort("category")}>Category</th>
              <th className="p-5 cursor-pointer" onClick={() => handleSort("priority")}>Priority</th>
              <th className="p-5 cursor-pointer" onClick={() => handleSort("due_date")}>Due Date</th>
        ...
        {!loading && filteredTickets.length === 0 && <div className="p-10 text-center text-gray-500">No tickets found</div>}
