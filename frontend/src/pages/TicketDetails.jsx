...
export default function TicketDetails() {
  ...
  const updateDueDate = async () => {
    try {
      const res = await api.put(`/tickets/${ticket.id}`, { due_date: dueDate, comment });
      setTicket(res.data);
      setTimeline(res.data.timeline || []);
      setComment("");
  ...
  const updateStatus = async () => {
    try {
      const res = await api.put(`/tickets/${ticket.id}`, { status, comment });
      setTicket(res.data);
      setTimeline(res.data.timeline || []);
      setComment("");
  ...
  const handleApproval = async (approvalStatus) => {
    try {
      const res = await api.put(`/tickets/${ticket.id}/approve`, { approval_status: approvalStatus });
      setTicket(res.data);
      setTimeline(res.data.timeline || []);
      ...
      <div className="bg-white rounded-3xl shadow-sm p-8">
        <div className="flex flex-col lg:flex-row justify-between lg:items-start gap-6">
          <div>
            ...
            <p className="text-gray-500 mt-2 break-all">Ticket ID: {ticket.id}</p>
          ...
          {user?.role === "Admin" && (
            <button onClick={deleteTicket} className="bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-2xl font-semibold">
              Delete Ticket
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mt-12">
          <div className="space-y-8">
            <div><p className="font-semibold text-gray-500">Description</p><p className="text-lg lg:text-xl font-medium mt-2">{ticket.description}</p></div>
            <div><p className="font-semibold text-gray-500">Priority</p><p className="text-lg lg:text-xl font-medium mt-2">{ticket.priority}</p></div>
            <div><p className="font-semibold text-gray-500">Category</p><p className="text-lg lg:text-xl font-medium mt-2">{ticket.category}</p></div>
            <div><p className="font-semibold text-gray-500">Due Date</p><p className="text-lg lg:text-xl font-medium mt-2">{ticket.due_date || "Not set"}</p></div>
          </div>
          <div className="space-y-8">
            <div><p className="font-semibold text-gray-500">Assigned To</p><p className="text-lg lg:text-xl font-medium mt-2">{ticket.assigned_to_name || "Unassigned"}</p></div>
            <div>
        ...
        {user?.role === "Admin" && (
          <div className="mt-16 border-t pt-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button onClick={assignTicket} className="bg-black hover:bg-gray-800 text-white px-8 py-3 rounded-2xl">
                Assign
              </button>
            </div>
          </div>
                  ...
                  {item.mentions?.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-4">
                      {item.mentions.map((mention, i) => (
                        <span key={i} className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs">{mention}</span>
                      ))}
                    </div>
                  )}
